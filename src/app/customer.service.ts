...
loadItemฺByComp(compCode): Observable<any> {
    //เรียก localhost ของ ตัว Api company ใน project ISsue Api Day 4
    //reactiveX เสร็จแล้วจะบอก ทำแบบไม่รอกัน

    return this.http.get( `${environment.apiUrl}/customer/findByIdAll/${compCode}`)
      .map((res: Response) => { 
  //  ส่งผลกลับจากตรงนี้
        return  res.json() 
      })
...
      .catch((error: any) => Observable.throw(error.json().error || 'Server error'));
  ...
  addItem(body): Observable<any> {
    ...
    return this.http.post( 
      `${environment.apiUrl}/customer`,body,options) // ...using post request
      .map((res: Response) => {
...
  }
  ...
  updatebyItem(body,id): Observable<any> {
    ...
    return this.http.put( 
      `${environment.apiUrl}/customer/${id}`,body,options) // ...using post request
      .map((res: Response) => {
...
SearchItem(body): Observable<any> {
    ...
    return this.http.post( 
      `${environment.apiUrl}/customer/search`,body,options) // ...using post request
      .map((res: Response) => {
